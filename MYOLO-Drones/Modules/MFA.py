...
class GateBlock(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.dwconv_3x3 = DepthwiseSeparableConv(hidden_features, hidden_features, kernel_size=3, padding=1)

        self.dwconv_5x5 = DepthwiseSeparableConv(hidden_features, hidden_features, kernel_size=5, padding=2)
...
class LLF(nn.Module):
    def __init__(self, in_channels):
        ...
        self.fusion = nn.Conv2d(in_channels * 4, in_channels, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        b1 = self.branch1(x)
        b2 = self.branch2(x)
        b3_h = self.branch3_h(x)
        b3_v = self.branch3_v(x)
        b4 = self.branch4(x)
...
class TinyLLF(nn.Module):
    def __init__(self, in_features, hidden_features=None, act_layer=nn.GELU, drop=0):
        ...
        self.depthwise_conv5x5 = nn.Sequential(
            nn.Conv2d(in_features, in_features, kernel_size=5, padding=2, groups=in_features, bias=False),
            nn.BatchNorm2d(in_features),
            act_layer(),
            nn.Conv2d(in_features, hidden_features, kernel_size=1, bias=False)
        )
        ...
        self.fc_out = nn.Conv2d(hidden_features * 2, in_features, kernel_size=1)
        self.drop = nn.Dropout(drop)

    def forward(self, x):
        input = x
        x2 = self.depthwise_conv3x3(x)
        x3 = self.depthwise_conv5x5(x)
...
class TMFA(nn.Module):
    def __init__(
        ...
        self.tinyllf = TinyLLF(hidden_dim)
        self.mlp_branch = mlp_ratio > 0
        if self.mlp_branch:
            self.norm2 = norm_layer(hidden_dim)
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
            self.mlp = GateBlock(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer,
                               drop=mlp_drop_rate)

    def forward(self, input):
        input = self.in_proj(input)
        # ====================
        X1 = self.tinyllf(input)
